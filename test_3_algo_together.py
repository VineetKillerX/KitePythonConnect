    ...
    while True:
        ...
        if(min%5==0 and (last_min==-1 or (min!=last_min and last_min!=-1))):
            ...
            if (datetime_obj.hour == 15 and datetime_obj.minute>15):
                ...
                trade_one(last_min,datetime_obj_hour_fwd,datetime_obj,min,token,historical_data_rsi,last_price,rsi,signal,suptrenval,holding,index,order_id,price)
                rsi_2=trade_two(last_min,datetime_obj_hour_fwd,datetime_obj,min,token,historical_data_rsi,last_price,rsi_2,signal,suptrenval,holding,index,order_id,price)
                trade_three(last_min,datetime_obj_hour_fwd,datetime_obj,min,token,historical_data_rsi,last_price,rsi,signal,suptrenval,holding,index,order_id,price)
            write_log(str(rsi)+","+str(signal)+","+str(signal2)+","+str(suptrenval)+","+str(suptrenval2)+","+str(datetime_obj)+"\n",rs1_trend_log)
        else:
            price = get_price_v1(token,last_price)
            for i in ["_1","_2","_3"]:
                global file_name
                file_name=csv_mapping[i]
                stoper(token,last_price[num],profit,stop_loss,datetime_obj,holding[num],order_id[num],price[num])
        time.sleep(1)      

def trade_one(last_min,datetime_obj_hour_fwd,datetime_obj,min,token,historical_data_rsi,last_price,rsi_2,signal,suptrenval,holding,index,order_id,price):
    num='_1'
    global file_name
    file_name=csv_mapping[num]
    place_orders_2(signal,suptrenval,token,holding[num],last_price[num],rsi,index,order_id[num],price[num])
      
def trade_two(last_min,datetime_obj_hour_fwd,datetime_obj,min,token,historical_data_rsi,last_price,rsi_2,signal,suptrenval,holding,index,order_id,price): #rsi_2 is equivalent to rsi on 1 hours
    num='_2'
    ...
    if(last_min==-1 or (datetime_obj_hour_fwd<=datetime_obj and min==15)):
        ...
        latest_data = df_rsi.tail(1).to_dict()
        rsi2=tail_dict['RSI_'+str(rsi_prop["range"])][index]
        datetime_obj_hour_fwd=getDateTime()+timedelta(hours=1)
    
    place_orders_2(signal,suptrenval,token,holding[num],last_price[num],rsi_2,index,order_id[num],price[num])   
    return rsi_2


def trade_three(last_min,datetime_obj_hour_fwd,datetime_obj,min,token,historical_data_rsi,last_price,rsi,signal,suptrenval,holding,index,order_id,price): 
    num="_3"
    global file_name
    file_name=csv_mapping[num]
    place_orders_3(signal,signal2,suptrenval,suptrenval2,token,holding[num],last_price[num],rsi,index,order_id[num],price[num])
