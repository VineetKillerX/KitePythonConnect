...
def write_log(action, name=file_name):
    global profit, stop_loss, historical_data, supertrend, rsi, rsi_slope, wma20, wma5, wma14, last_close, last_price, order_id, holding,current_price
    log = ''
...
def get_history_data():
    global historical_data, supertrend, rsi, rsi_slope, wma20, wma5, wma14, last_close, last_price, swing,last_high,last_low
    swing = False
    ...
    print(rsi)
    print(rsi_slope)
    print("Values in History Data is "+str(supertrend) + ',' + str(rsi) + ',' + str(rsi_slope) + ',' + str(
        wma20) + ',' + str(wma5) + ',' + str(wma14) + ',' + str(last_close))
...
def trade():
    ...
    while True:
        datetime_obj = getDateTime()
        if (datetime_obj.hour >= 15 and datetime_obj.minute > 26) or (
        ...
        if minutes % 5 == 0 and (last_min == -1 or (minutes != last_min and last_min != -1)):
            ...
            if minutes == 15 and datetime_obj.hour == 9:
                if (abs(last_high - last_low) / last_close) >= 0.01:
                    time.sleep(5 * 60)
            place_order(signal)
            last_min = minutes
        else:
